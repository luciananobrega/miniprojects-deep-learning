...
def test_generator(generator, epoch):
    with torch.no_grad():
        ...
        fake_images = generator(fake_data)
        fake_images = fake_images.view(BATCH_SIZE, 3, 32, 32)[:64]
...
if __name__ == '__main__':
    ...
    for i, param in enumerate(params):
        ...
        OUT_FOLDER = './output/gan_cifar_' + str(param).replace('(', '').replace(')', '').replace(', ', '_')
        ...
        gen_model = Generator(CIFAR_SIZE, latent_size, n_layers)
        dis_model = Discriminator(CIFAR_SIZE, n_layers)
