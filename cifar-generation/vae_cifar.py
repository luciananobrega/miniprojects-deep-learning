...
def train_vae(model):
    ...
    for batch_idx, (data, _) in enumerate(tqdm(train_loader)):
        data = data.view(-1, CIFAR_SIZE).to(device)  # Flatten CIFAR images
        optimizer.zero_grad()
...
def evaluate_vae(model, epoch):
    ...
    with torch.no_grad():
        for batch_idx, (data, _) in enumerate(tqdm(test_loader)):
            data = data.view(-1, CIFAR_SIZE).to(device)  # Flatten MNIST images
            reconstruction, mu, log_var = model(data)
            bce_loss, kl_loss, loss = vae_loss(reconstruction, data, mu, log_var)
            running_loss.append(loss.item())
            running_loss_bce.append(bce_loss.item())
            ...
            if batch_idx == int(len(test_loader.dataset) / BATCH_SIZE) - 1:
                recon_batch_ = reconstruction.view(BATCH_SIZE, 3, 32, 32)[:64]
...
if __name__ == '__main__':
    ...
    for i, param in enumerate(params):
        ...
        OUT_FOLDER = './output/vae_cifar_' + str(param).replace('(', '').replace(')', '').replace(', ', '_')
        ...
        vae_model = VAE(CIFAR_SIZE, latent_size, n_layers)
